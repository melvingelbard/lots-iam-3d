    ...
    for ii_s in range(0, len(num_samples_all)):
        ...
        with open(csv_filename, newline='', encoding='utf-8-sig') as csv_file:
            ...
            for row in reader:            
                ...
                ## Remove empty slices at the top and bottom of mri volume
                index_start = 0
                index_end = z_len-1
                ...
                for xyz in range(0, len(patch_size)):
                    ...
                    vol_slice = np.count_nonzero(mri_data) / whole_volume                       ## Proportion of brain slice compared to full image
                    print('DEBUG-Patch: brain_slice - ' + str(np.count_nonzero(mri_data)) + 
                    ', x_len * y_len * z_len - ' + str(whole_volume) + ', vol: ' + str(round(vol_slice, 5)))       ## x_len/y_len = 512 here
                    ...
                    source_patch_len = counter_x * counter_y * counter_z                                ## How many source patches are needed (e.g. for 1, we need one for each pixel)
                    ...
                    volume_source_patch = np.zeros([1,patch_size[xyz],patch_size[xyz], patch_size[xyz]])
                    ...
                    for isc in range(0, counter_x):
                        for jsc in range(0, counter_y):
                            for ksc in range(0, counter_z):
                                if source_patch_len/2 == index:
                                    print("50% of source patches extracted.")
                                icv_source_flag[index] = mri_mask[int(x_c_sources[isc]), int(y_c_sources[jsc]), int(z_c_sources[ksc])]

                                if icv_source_flag[index] == 1:
                                    if flag:
                                        flag = 0
                                        temp = np.matrix([x_c_sources[isc], y_c_sources[jsc], z_c_sources[ksc]])
                                        ...
                                        volume_source_patch[0,:,:] = temp
                                    else:
                                        temp = np.matrix([x_c_sources[isc], y_c_sources[jsc], z_c_sources[ksc]])
                                        center_source_patch = np.concatenate((center_source_patch, temp))
                                        temp = get_volume(x_c_sources[isc], y_c_sources[jsc], z_c_sources[ksc],
                                                        patch_size[xyz], patch_size[xyz], patch_size[xyz], mri_data)
                                        temp = np.reshape(temp, (1, patch_size[xyz], patch_size[xyz], patch_size[xyz]))
                                        volume_source_patch = np.concatenate((volume_source_patch, temp))
                    ...
                    for iii in range(0, x_len):
                        for jjj in range(0, y_len):
                            for kkk in range(0, z_len):
                                index_population += 1
                                if mri_mask[iii,jjj,kkk] != 0 and np.random.rand(1) > TRSH:
                                    volume = get_volume(iii, jjj, kkk, patch_size[xyz], patch_size[xyz], patch_size[xyz], mri_data)
            ...
            for zz in range(0, mri_data.shape[2]): 
                ...
                for xyz in range(0, len(patch_size)):
                    ...
                    slice_age_map_res = cv2.resize(slice_age_map, None, fx=patch_size[xyz],
                                                    fy=patch_size[xyz], interpolation=cv2.INTER_CUBIC)
                    ...
                    #code.interact(local=dict(globals(), **locals()))
                    slice_age_map_all[xyz,:,:] = slice_age_map_res
